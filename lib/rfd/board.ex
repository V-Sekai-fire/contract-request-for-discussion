  ...
  def check(entries \\ Corpus.entries()) do
    ...
    steps =
      for {serial, i} <- Enum.with_index(@critical_path) do
        case by[serial] do
          nil -> "critical-path step #{i} names RFD #{serial}, which has no source"
          %{flight_level: l} when l != :l1 -> "critical-path RFD #{serial} is #{l || "unlevelled"}, not L1"
          %{state: s} when s in [:abandoned, :moved] -> "critical-path RFD #{serial} is #{s}"
          _ -> nil
        end
      end
    ...
    order = if ranks == Enum.sort(ranks), do: [], else: ["register levels are not ordered L1, L2, L3, then unlevelled"]
