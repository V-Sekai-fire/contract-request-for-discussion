...
`CLAUDE.md` carries the allowlist, and anything not on it is denied by default. This file keeps
the explicit denials: the list below, each with its one-line reason, and the argument behind
each entry that says "see below".

Neither half of this file may drift from the other. `scripts/check_blocklist_detail.py`
requires that every entry saying "see below" resolves to a section here, and that every section
here corresponds to an entry.
...
- **Krea 2 / krea2-turbo**: revenue-gated licence — see below
...
- **`giraffe.glb`** (SkinTokens example asset): bundled art with no stated provenance; the repo's MIT covers its code — see below
- **`jp.lilxyzw.ndmfmeshsimplifier`**: operator directive 2026-09-12: decimation deferred to build time, which is the property just removed from the avatar — see below
- **git submodules**: a second dependency mechanism `repo status` cannot see — use `default.xml`, see below
- **Windows** as a host for workspace tooling: operator directive 2026-09-25, reversing the WSL row: tooling runs under WSL; a published Windows artifact is still tested on Windows — see below
...
The licence reason stands on its own, and hardware does not move it. A 48 GB
card running bf16 would still leave the revenue gate.
