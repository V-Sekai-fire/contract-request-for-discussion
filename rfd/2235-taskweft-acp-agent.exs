...
defmodule RFD2235 do
  ...
  rfd 2235, "An ACP agent that plans with taskweft and executes through the editor" do
    ...
    feature "a no-model Agent Client Protocol agent: a deterministic " <>
              "grammar, taskweft plans, ACP tool calls under the client's " <>
              "permission, the session as an event log in the fdb-sqlite store"

    scope "`1-transport/transport-taskweft-acp`, the Fly app " <>
            "`weftspun-taskweft-acp`, the executor sides that dial into it"
    ...
    never GitHub, and runs no step itself: executor sides dial in under a
    bao-held policy with a local override. `DETAILS.md` carries the rest.
    """
    ...
    related ~S"""
    RFD 2234 (the dress-on plan this repository's planning pattern was first tried on), RFD 2111
    (the side the repository sits on), RFD 2134 (the cluster's TLS).
    """
