...
of its named consolidations landing)"
    ...
    **Layer 3, game / video code.** Consumers instantiate adapters
    ...
    |---|---|---|
    | motion-bricks-cpp | own module, pin, CMake | shared `modules/ggml/` + `motion_bricks.gd` |
    | kimodo | own module, ggml pin | shared `modules/ggml/` + `kimodo.gd` |
    | skin-tokens-cpp | own module, ggml pin | shared `modules/ggml/` + `skin_tokens.gd` |
